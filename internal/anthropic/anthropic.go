...
// Block is one assembled content block, complete. tokentrace's sidecar kept only
// the *type* of a thinking block and stringified tool inputs — and so could
// never answer a question it hadn't already thought of. This one keeps it all.
type Block struct {
	Type      string          `json:"type"`
